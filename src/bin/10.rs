...
fn find_paths(map: &Vec<Vec<u32>>, cur_path: Vec<(isize, isize)>) -> Vec<(isize, isize)> {
    ...
    .iter()
    {
        if *next_row >= 0
            ...
            && !cur_path.contains(&(*next_row, *next_col))
        {
...
pub fn part_one(input: &str) -> Option<u32> {
    ...
    for (row, col) in trailheads {
        ...
        let reachable_set: HashSet<(isize, isize)> =
            HashSet::from_iter(reachable.iter().map(|x| *x));
        score += reachable_set.len();
        println!(
            "trailhead: {:?}, reachable: {:?} ({}), running score: {}",
            (row, col),
            reachable_set,
            reachable_set.len(),
            score
...
pub fn part_two(input: &str) -> Option<u64> {
    ...
    for (row, col) in trailheads {
        ...
        score += path_count;
        println!(
            "trailhead: {:?}, path_count: {}, running score: {}",
            (row, col),
            path_count,
            score
        );
